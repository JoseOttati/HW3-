 ...
 */
 ...
 class LUC_AVLTree {
    ...
    if (node == null) {
        return node; // If the node is null, return null (nothing to delete)
    }

    // Recursively find the node to delete
    if (value < node.value) {
        node.leftChild = deleteElement(value, node.leftChild); // Search in left subtree
    } else if (value > node.value) {
        node.rightChild = deleteElement(value, node.rightChild); // Search in right subtree
    } else {
        // Node to be deleted found, handle deletion
        if (node.leftChild == null || node.rightChild == null) {
            // If the node has one or no child, use the child (or null) as replacement
            Node temp = (node.leftChild != null) ? node.leftChild : node.rightChild;
            if (temp == null) {
                // If no children, set node to null
                node = null;
            } else {
                // Otherwise, replace node with the non-null child
                node = temp;
            }
        } else {
            // Node with two children: Get the inorder successor (minimum in right subtree)
            Node temp = minValueNode(node.rightChild);
            node.value = temp.value; // Copy inorder successor’s value to this node
    ...
    // Step 4: Perform AVL rotations if needed

    // Left Heavy (Right Rotation - LL)
    if (balance > 1 && getBalanceFactor(node.leftChild) >= 0) {
        return LLRotation(node);
    }

    // Left Right Heavy (Left-Right Rotation - LR)
    if (balance > 1 && getBalanceFactor(node.leftChild) < 0) {
        node.leftChild = RRRotation(node.leftChild); // Left rotation on the left child
        return LLRotation(node); // Right rotation on current node
    }

    // Right Heavy (Left Rotation - RR)
    if (balance < -1 && getBalanceFactor(node.rightChild) <= 0) {
        return RRRotation(node); // Perform a right rotation
    }

    // Right Left Heavy (Right-Left Rotation - RL)
    if (balance < -1 && getBalanceFactor(node.rightChild) > 0) {
        node.rightChild = LLRotation(node.rightChild); // Left rotation on the right child
        return RRRotation(node); // Right rotation on current node
    }
